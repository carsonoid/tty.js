...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('setShellArgs', function(args) {
    return session.handleSetShellArgs(args);
  });

  socket.on('authenticate', function(key, func) {
    return session.handleAuthenticate(key, func);
  });
...
Session.prototype.handleSetShellArgs = function(args) {
  var cleanargs = [];
  for (var i = args.length - 1; i >= 0; i--) {
    cleanargs.push(paranoidSanitize(args[i]));
  };
  this.server.conf.shellArgs = cleanargs;
};

Session.prototype.handleAuthenticate = function(key, func) {
  if (this.server.conf.key == key) {
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  if (!self.authenticated) {
    this.warning('Session not authenticated.');
    return func({ error: 'Not authenticated.' });
  }
...
function paranoidSanitize(text) {
